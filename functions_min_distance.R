#' Function to add binodal values
#'
...
#'
#' @examples
f.add.binodal <- function(l.in = list(), x_data = "RFPconc", y_data = "GFPconc", diagonal_distance = 1, bandwidth = 0.5, peak_range_lim, peak_method = "largest"){
  for (name in names(l.in)) {
    ...
    l.in[[name]][["density"]]  <- density(l.in[[name]][["avg_dist"]][l.in[[name]][["close_to_diag"]]], bw = bandwidth)
    
    l.in[[name]][["peak_and_half"]]  <-  get_peak(d = l.in[[name]]$density, peak_range = peak_range_lim, method = peak_method)
  }
  l.in
}
...
#' @param peak_range Given prior knowledge, in which range is peak of interest. Specifiy vector
...
get_peak <- function(d, peak_range, method = "largest"){
  ...
  if (method == "last") { #get last (rightmost) peak, i.e. highest x-value
  ...
  } else{ #get largest peak, i.e. highest y-value
